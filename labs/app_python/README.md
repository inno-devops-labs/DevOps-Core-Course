...
| Issue | Solution |
|-------|----------|
| Port already in use | Use `PORT=8080 python app.py` to change the port |
| Module not found error | Activate virtual environment and run `pip install -r requirements.txt` |
| Service not accessible | Check firewall settings or use `HOST=localhost` for local-only access |

---

## Development Notes

- The service tracks uptime from the moment the application starts
- All timestamps are in UTC format (ISO 8601)
- Request information is captured from the incoming HTTP request headers
- CPU count and architecture are detected from the system automatically
